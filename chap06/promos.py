...
print(globals())
#! memo ************ 0006  关于对字典的迭代 ************
#! memo 方法1, 保证迭代时不要改变字典的长度 
#!            如果没有 name = None,  
#!            会报错: "RuntimeError: dictionary changed size during iteration
#!            因为对 globals() 进行迭代时, 引入了 name, 
#!            globals() 这个字典大小改变了 在迭代的时候

print('----------- 方法1 declare iter var before use globals() -----------')
iter_var = None # 若没有这行,则迭代时字典大小改变
for iter_var in globals().keys():
    if not iter_var == "iter_var":
        print(iter_var)

print('----------- 方法2 force copy by using list() -----------')
#! memo 方法2, 直接对原字典迭代可能改变字典的长度, 那就单独深拷贝一份出来用做迭代之用
for iter_var in list(globals()):
    if not iter_var == "iter_var":
        print(iter_var)

print('----------- 方法3 列表推导式自带拷贝  -----------')

print(*[ name for name in globals().keys() if not name=='name'], sep = "\n")
